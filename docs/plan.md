...
状态（2026-09-25）：**M1–M6 的完成判据全部达成**。M5 的浏览器实测、图片输入与真实 Harness 进程内的会话头抓包见 `docs/verification-m5-live.md`（含实机发现并修复的两个客户端缺陷：catalog Remote 未挂载导致设置分区不注册、凭证结果未按 `RemoteResult` 信封解包导致 API Key 误报）。
